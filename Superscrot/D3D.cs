...
public interface IDirect3DDevice9
{
    ...
    int GetGammaRamp();
    [PreserveSig]
    int CreateTexture(int width, int height, uint levels, uint usage, uint format, int pool, [Out] out IntPtr pSurface, [In, Out]ref IntPtr ppSurface);
    ...
    [PreserveSig]
    int CreateRenderTarget(int width, int height, uint format, D3DMULTISAMPLE_TYPE multisample, uint multisamplequality, bool lockable, [Out]out IntPtr surface, [In, Out]ref IntPtr pSharedSurface);
    ...
    int ColorFill();
    [PreserveSig]
    int CreateOffscreenPlainSurface(int width, int height, uint format, int pool, [Out] out IntPtr pSurface, [In, Out]ref IntPtr ppSurface);
...
public interface IDirect3D9Ex : IDirect3D9
{
    ...
    int GetAdapterLUID(uint Adapter, [In, Out] ref Int64 pLUID);
}
